...
public class HouseDataManager {
	...
	private List<House> loadHouses() {
		...
		String color[] = {"yellow-orange", "green-silver", "silver-yellow", "brown-blue"};
		String mascot[] = {"Lion", "Serpent", "Badger", "Eagle"};
		...
		for (int index=0; index < houseNames.length; index++) {
			...
			house.setColor(color[index]);
			house.setMascot(mascot[index]);
